...
def create_interpretability_report(importance_df, save_path='../paper'):
    ...
    importance_df.to_csv(
        os.path.join(save_path, 'shap_feature_importance.csv'),
        index=False
    )
    ...
    report.append(f"  {top_feature} is the dominant driver with mean |SHAP| = {top_importance:.4f}")
    report.append(f"  This confirms the critical role of geopolitical factors in volatility forecasting.")
...
def run_full_explainability_pipeline(model, X_train, X_test, feature_names, 
