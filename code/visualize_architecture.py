...
def create_architecture_diagram(save_path='../figures/model_architecture.png'):
    ...
    # Title
    ax.text(5, 11.5, 'LSTM-Attention-SHAP Architecture', 
            ha='center', fontsize=18, fontweight='bold')
    
    # Layer definitions with positions
    layers = [
        {'name': 'Input\n(30, 15)', 'pos': (5, 10), 'color': '#E8F4F8'},
        {'name': 'LSTM Layer 1\n128 units\ndropout=0.2', 'pos': (5, 8.5), 'color': '#B8E6F0'},
        {'name': 'LSTM Layer 2\n64 units\ndropout=0.2', 'pos': (5, 7), 'color': '#88D8E8'},
        {'name': 'Attention Layer\nBahdanau-style\n64 units', 'pos': (5, 5.5), 'color': '#FFD88F'},
        {'name': 'Context Vector\nWeighted Sum', 'pos': (5, 4), 'color': '#FFC470'},
        {'name': 'Dense Layer\n32 units + ReLU\ndropout=0.3', 'pos': (5, 2.5), 'color': '#D4A5FF'},
    ]
    ...
    for output in outputs:
        ...
        ax.add_patch(box)
        ax.text(output['pos'][0], output['pos'][1], output['name'],
                ha='center', va='center', fontsize=10, fontweight='bold')
    ...
    ax.annotate('Attention Weights\n(Dynamic Focus)', 
                xy=(5, 5.5), xytext=(8.5, 5.5),
                fontsize=9, ha='left',
                bbox=dict(boxstyle='round,pad=0.5', facecolor='yellow', alpha=0.3),
                arrowprops=dict(arrowstyle='->', lw=1.5, color='orange'))
    
    # Add SHAP annotation
    ax.annotate('SHAP Layer\n(Explainability)', 
                xy=(5, 2.5), xytext=(1.5, 2.5),
                fontsize=9, ha='right',
    ...
    ax.text(0.3, 6.5, spec_text, fontsize=9, 
    ...
    ax.text(9.7, 6.5, loss_text, fontsize=9,
